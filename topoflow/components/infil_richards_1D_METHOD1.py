...
class infil_component(infil_base.infil_component):
    ...
    def initialize_layer_vars(self):
        ...
        #-------------------------------------------------
        self.soil_type = numpy.zeros(n_layers, dtype='|S100')
        self.dz_val    = numpy.zeros(n_layers, dtype='Float64')    #### + dz3
        self.nz_val    = numpy.zeros(n_layers, dtype='Int16')      #### + nz3
        #--------------------------------------------------------
        self.Ks_type   = numpy.zeros(n_layers, dtype='|S100')
        self.Ki_type   = numpy.zeros(n_layers, dtype='|S100')
        self.qs_type   = numpy.zeros(n_layers, dtype='|S100')
        self.qi_type   = numpy.zeros(n_layers, dtype='|S100')
        self.qr_type   = numpy.zeros(n_layers, dtype='|S100')
        self.pB_type   = numpy.zeros(n_layers, dtype='|S100')    
        self.pA_type   = numpy.zeros(n_layers, dtype='|S100')
        self.lam_type  = numpy.zeros(n_layers, dtype='|S100')
        self.c_type    = numpy.zeros(n_layers, dtype='|S100')
        #--------------------------------------------------------        
        self.Ks_file  = numpy.zeros(n_layers, dtype='|S100')
        self.Ki_file  = numpy.zeros(n_layers, dtype='|S100')
        self.qs_file  = numpy.zeros(n_layers, dtype='|S100')
        self.qi_file  = numpy.zeros(n_layers, dtype='|S100')
        self.qr_file  = numpy.zeros(n_layers, dtype='|S100')
        self.pB_file  = numpy.zeros(n_layers, dtype='|S100')
        self.pA_file  = numpy.zeros(n_layers, dtype='|S100')
        self.lam_file = numpy.zeros(n_layers, dtype='|S100')
        self.c_file   = numpy.zeros(n_layers, dtype='|S100')
        #---------------------------------------------------------
        # Note: self.Ks is a Python list.  Initially, each entry
        # is a numpy scalar (type 'numpy.float64').  However, we
        # can later change any list entry to a scalar or grid
        # (type 'numpy.ndarray'), according to its "Ks_type".
        #---------------------------------------------------------
        # (5/19/10) Seems we need Ks_val vs Ks here, since
        # we use these to build one big, 3D Ks array.
        #---------------------------------------------------------        
        self.Ks_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.Ki_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.qs_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.qi_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.qr_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.pB_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.pA_val  = list(numpy.zeros(n_layers, dtype='Float64'))
        self.lam_val = list(numpy.zeros(n_layers, dtype='Float64'))
        self.c_val   = list(numpy.zeros(n_layers, dtype='Float64'))
        #------------------------------------------------
        # Note:  These two are computed from the others
        #------------------------------------------------
        self.eta_val = list(numpy.zeros(n_layers, dtype='Float64'))
        self.qH_val  = list(numpy.zeros(n_layers, dtype='Float64'))
...
##    #   read_cfg_file()
    ...
    def check_input_types(self):
        ...
        self.ALL_SCALARS = numpy.all(are_scalars)
...
##        print '#### In check_input_types(), ALL_SCALARS =', self.ALL_SCALARS
    ...
    def initialize_richards_vars(self):
        ...
        if (self.DEBUG):
            ...
            print 'shape(Ks)   =', numpy.shape(self.Ks)
            print 'shape(Ki)   =', numpy.shape(self.Ki)
            print 'shape(qs)   =', numpy.shape(self.qs)
            print 'shape(qi)   =', numpy.shape(self.qi)
            print 'shape(qr)   =', numpy.shape(self.qr)
            print 'shape(pB)   =', numpy.shape(self.pB)
            print 'shape(pA)   =', numpy.shape(self.pA)
            print 'shape(lam)  =', numpy.shape(self.lam)
            print 'shape(c)    =', numpy.shape(self.c)
            print 'shape(eta)  =', numpy.shape(self.eta)
            print 'shape(qH)   =', numpy.shape(self.qH)
            print ' '
        ...
        if (dz_min == dz_max):
            ...
            # dz is a 1D array
            #-------------------
            self.dz = numpy.zeros(self.nz, dtype=dtype)
        ...
        for j in xrange(1, self.n_layers):
            ...
            dz_j = repeat(layer_dz, layer_nz)  # (1D ndarray)
            dz = numpy.concatenate( (dz, dz_j) )
        ############################################
        # NB! As written (and in IDL version), the
        #     z-vector does not start with 0.
        ############################################
        self.z = numpy.cumsum(dz)
        ...
        if (self.ALL_SCALARS):
            ...
            #----------------------------------
            self.q = numpy.zeros(self.nz, dtype=dtype) + self.qi
            self.p = numpy.zeros(self.nz, dtype=dtype)
            self.K = numpy.zeros(self.nz, dtype=dtype) + self.Ki
            self.v = numpy.zeros(self.nz, dtype=dtype)
            #---------------------------------------------------
            self.IN = numpy.float64(0)   # (infil. rate at surface)
            self.I  = numpy.float64(0)   # (total infil. depth)
            self.Zw = numpy.float64(0)   # (wetting front depth)
...
##                print 'type(P_total)  =', type(self.P_total)
            ...
            #---------------------------------------------------------------
            self.IN = numpy.zeros([self.ny, self.nx], dtype=dtype)
            self.I  = numpy.zeros([self.ny, self.nx], dtype=dtype)
            self.Zw = numpy.zeros([self.ny, self.nx], dtype=dtype)

            #--------------------------------------
            # Initialize q to qi (qi is 1D or 3D)
            #--------------------------------------
            if (numpy.size(self.qi) == self.nz):
            ...
            #--------------------------------------------------
            ## if (numpy.ndim(self.qs) == 1):
            if (numpy.size(self.qs) == self.nz):
                temp = self.qs.copy()
            ...
            #--------------------------------------------------
            ## if (numpy.ndim(self.qH) == 1):
            if (numpy.size(self.qH) == self.nz):
                temp = self.qH.copy()
...
##        print 'min(IN), max(IN) =', self.IN.min(), self.IN.max()
    ...
    def update_theta(self, REPORT=False):
        ...
        if (self.SINGLE_PROFILE):    
            ...
            #-------------------------------------------------
            dv_dz[self.nz - 1] = numpy.float64(0)
        ...
        #---------------------------------------------------
##        self.q = numpy.minimum( self.q, self.qs )
##        self.q = numpy.maximum( self.q, self.qH )
    ...
    def check_theta(self):
        ...
        w = numpy.where( logical_or( (self.q < self.qH),
                                     (self.q > self.qs)) )
    ...
    def update_psi(self, REPORT=False):
        ...
        if (self.SINGLE_PROFILE):    
            ...
            #--------------------------------------
            dim_qs  = numpy.ndim(self.qs)
            dim_qr  = numpy.ndim(self.qr)
            dim_pB  = numpy.ndim(self.pB)
            dim_pA  = numpy.ndim(self.pA)
            dim_lam = numpy.ndim(self.lam)
            dim_c   = numpy.ndim(self.c)
    ...
    def update_K(self, REPORT=False):
        ...
        if (self.SINGLE_PROFILE):    
            ...
            # Each var is either a 1D or 3D array
            #--------------------------------------
            dim_Ks  = numpy.ndim(self.Ks)
            dim_pB  = numpy.ndim(self.pB)
            dim_pA  = numpy.ndim(self.pA)
            dim_eta = numpy.ndim(self.eta)
            dim_c   = numpy.ndim(self.c)
...
##        print 'nz             =', self.nz
##        print 'size(self.dz)  =', numpy.size(self.dz)
##        print ' '
        ...
        if (self.SINGLE_PROFILE):    
            ...
            K_below = numpy.roll(self.K, -1, axis=0)
            K_bar   = (self.K + K_below) / 2.0
            vB = K_bar * (1.0 - dp_dz)      # (bottom of cell)
            vT = numpy.roll(vB, 1, axis=0)  # (top of cell)
...
##        print 'shape(qs) =', numpy.shape(self.qs)
##        print 'shape(q)  =', numpy.shape(self.q)
##        print 'shape(vT) =', numpy.shape(vT)
##        print 'shape(vB) =', numpy.shape(vB)
##        print 'shape(v)  =', numpy.shape(self.v)
        ...
        if (n_filling != 0):
            ...
            # print 'shape(space_avail) =', numpy.shape(space_avail)
...
##        print 'shape(v)  =', numpy.shape(self.v)
    ...
    def update_Zw(self, REPORT=False):
        ...
        if (self.SINGLE_PROFILE):    
            q_below = numpy.roll(self.q, -1, axis=0)
            diff = (self.q - q_below)
